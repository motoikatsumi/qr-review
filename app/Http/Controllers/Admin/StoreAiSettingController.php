...
class StoreAiSettingController extends Controller
{
    ...
    {
        return redirect("/admin/stores/{$store->id}/edit?tab=ai");
    }

    /**
     * AI 入力サポート：店舗名・業種・住所などから AI 設定の候補を生成
     */
    public function aiSuggest(Request $request, Store $store)
    {
        ...
        $context = "店舗名『{$storeName}』、業種『{$industry}』";
        ...
        try {
            ...
            if ($target === 'store_description' || $target === 'all') {
                $result['ai_store_description'] = $this->genText(
                    "{$context} の AI 文章生成用の『店舗自己紹介』を生成してください。例：『質屋・買取店 質屋アシスト のスタッフ』『地域密着型の家族経営の焼肉店 〇〇 のスタッフ』。文末は『…のスタッフ』で終わる 1 文。30 文字以内。"
                );
            }
            if ($target === 'custom_instruction' || $target === 'all') {
                $result['ai_custom_instruction'] = $this->genText(
                    "{$context} のお店の特徴・基本方針を AI に伝える 1〜2 文。例：『家族連れのお客様が多く、個室が自慢のお店です。』『高級路線の接客スタイルです。』。100 文字以内、店舗名は含めず特徴のみ。"
                );
            }
            if ($target === 'area_keywords' || $target === 'all') {
                $result['ai_area_keywords'] = $this->genLines(
                    "{$context} で使うエリア（地域）キーワードを 5〜10 個生成してください。例：『〇〇市』『△△駅前』『□□エリア』。住所が分からないので一般的な業種名+地域型のキーワードでも可。"
                );
            }
            if ($target === 'service_keywords' || $target === 'all') {
                $result['ai_service_keywords'] = $this->genLines(
                    "{$context} で扱う『サービスキーワード』を 5〜10 個生成してください。例（質屋なら『高価買取』『査定・鑑定』『無料査定』）（焼肉店なら『黒毛和牛』『個室席』『ランチコース』）。1 行 1 ワード。"
                );
            }
            if ($target === 'ng_words' || $target === 'all') {
                $result['ai_extra_ng_words'] = $this->genLines(
                    "{$context} で AI が口コミを書く時に避けたい『追加 NG ワード』を 5〜10 個。業種共通の禁止ワードに加える店舗固有のもの。例：『〇〇社（競合店名）』『個人名』『電話番号』。1 行 1 ワード。"
                );
            }
            if ($target === 'reply_instruction' || $target === 'all') {
                $result['ai_reply_instruction'] = $this->genText(
                    "{$context} で Google レビューに返信する際の AI への特別指示文を 1〜2 文で。例：『丁寧で温かみのある口調で、お客様への感謝を必ず最初に述べる。具体的なサービスへの言及があれば触れる。』。100 文字以内。"
                );
            }
            if ($target === 'custom_hashtags' || $target === 'all') {
                $result['custom_hashtags'] = $this->genLines(
                    "{$context} の Instagram/Facebook 投稿に付ける『店舗固有ハッシュタグ』を 5〜8 個生成してください。# は付けず単語のみ。例：『質屋アシスト鹿児島店』『鹿児島市天文館』『ブランド買取専門』。"
                );
            }

            return response()->json(['success' => true, 'data' => $result]);
        } catch (\Exception $e) {
            \Log::error('Store aiSuggest failed', ['store_id' => $store->id, 'error' => $e->getMessage()]);
    ...
    public function update(Request $request, Store $store)
    {
        ...
        if ($footerText) {
            $store->postTemplate()
                ->updateOrCreate(['store_id' => $store->id], ['template_text' => $footerText]);
        ...
        return redirect("/admin/stores/{$store->id}/edit?tab=ai")->with('success', 'AI設定を保存しました。');
    }
}
