...
EOT;
    ...
    public function generateSuggestion(Store $store, $keywords, string $gender = '', string $age = '', string $visitType = '', string $item = '', array $personaExtra = [], int $rating = 5): ?string
    {
        ...
        // 書き出し/締めのパターンをランダムに選んで構造を分散させる(同じ品目で似た文型に収束するのを防ぐ)
        $openingPatterns = [
            '気持ちや感想から書き始める(例: 「思っていたよりも〜」「想像以上に〜」)',
            '来店時の状況や場面から書き始める(例: 「先日〇〇に立ち寄ったところ〜」)',
            '結論・結果から書き始める(例: 「〇〇の対応が良かったです」)',
            '事前の不安や迷いから書き始める(例: 「初めは緊張していましたが〜」)',
            '比較・気づきから書き始める(例: 「他と比べても〜」「久しぶりに〜」)',
            'スタッフの印象から書き始める',
            '具体的なシーンの描写から始める(例: 「店内に入るとすぐに〜」)',
            '率直な一言から始める(例: 「とても満足できました」)',
            '時間の流れから書き始める(例: 「査定から受け取りまで〜」)',
            'リピートの動機から書き始める(例: 「いつもお願いしている〇〇〜」)',
        ];
        $closingPatterns = [
            'また利用したい気持ちで締める',
            '知人や家族にもすすめたい気持ちで締める',
            ...
            'ありがとうの言葉で締める',
            '次回も任せたい気持ちで締める',
            'シンプルに満足感を一言で締める(例: 「満足です」「良かったです」)',
            '具体的な場面の感想で締める(例: 「対応が丁寧で安心しました」)',
            '言い切りで締める(例: 「また来ます」「お願いしてよかったです」)',
            '安心感を伝える一言で締める',
            '気持ちのよい余韻を残す表現で締める',
        ];
        $openingPattern = $openingPatterns[array_rand($openingPatterns)];
        $closingPattern = $closingPatterns[array_rand($closingPatterns)];
...
- 店舗名(「{$storeName}」など)は本文中に絶対に書かない。背景情報として理解するだけにとどめ、「○○店」「○○アシスト」のような店舗を特定する表現は出力しない
★★★ ここまで ★★★

【店舗名(参考のみ・本文には書かない)】{$storeName}
...
- 【最重要・店舗名禁止】店舗名(「{$storeName}」「○○店」「○○アシスト」等)は文章中に絶対に書かない。お客様視点の自然な口コミは店舗名を明記しないのが普通
{$itemInstruction}
★★★ ここまで ★★★
...
【文章のトーン】{$tone}
【文章の切り口】{$focus}
【書き出しの型】{$openingPattern}
【締めの型】{$closingPattern}
{$customInstruction}
条件：
- 【最重要】{$lengthInstruction}
- 【最重要・冒頭】上記「書き出しの型」に従って書き出すこと。「{$keyword}は丁寧で〜」のような品目名+丁寧という定型から始めない
- 【最重要・締め】上記「締めの型」に従って締めること。「また利用させていただきます」「また利用したいと思います」のような決まり文句に毎回頼らない
- 同じテーマ・同じ品目でも、毎回まったく別のお客様が書いた口コミに見えるよう、文の構造・語彙・着眼点を意識的に変える
- 最後は「。」「！」「絵文字」のいずれかで終わる。絵文字で終わる場合は「。」や「！」は付けない。たまに句読点も絵文字もなく言い切りで終わってもよい（例：「また来たいです」）。「。！」「！。」のような句読を連続で使わない
- 「」（カギ括弧)は一切使わない
- 指定されたペルソナ・スタイル・トーン・切り口・書き出し・締めを反映した自然な口語体にする
