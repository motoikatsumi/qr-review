...
- 「最高額」「業界一」「日本一」などの誇大表現
- 不自然な役割語や方言的語尾（例：「〜ましたわ」「〜ですわ」「〜ますのよ」「〜でございますわ」「〜だわ」「〜るわ」「〜たわ」「〜のよね」「〜のよ」「〜かしら」）。語尾に「わ」を付ける表現は全て禁止
...
EOT;
    ...
    public function generateReplyComment(
        ...
        // 顧客タイプの指示
        $customerTypeInstruction = $customerType === 'repeater'
            ? '- お客様はリピーターです。「いつもご利用いただき」「この度も」「日頃より」など、リピーターであることが伝わる表現を冒頭に含める'
            : '- お客様は新規のお客様です。「この度は初めてのご来店」「初めてご利用いただき」など、初来店であることに触れる表現を冒頭に含める';

        // エリアキーワード（ランダムに1つ選んで地名バリエーションを出す）
        $areaKeywords = [
...
- 口コミ内容が空（星評価のみ）の場合は、ご来店と評価への感謝を丁寧に述べ、取扱商品の紹介を含める
